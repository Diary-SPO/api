...
const getCookieFromToken = async (token: string) => {
    ...
    const getCookieQueryBuilder = await createQueryBuilder<CookieGetDetailedInfo>(client)
        .select('auth.id', '"idDiaryUser"', 'token', '"lastDate"', '"lastUsedDate"', 'cookie')
    ...
    getCookieQueryBuilder.cookie = decrypt(getCookieQueryBuilder.cookie, ENCRYPT_KEY)
 ...
 * @returns {void}
 */
const taskScheduler = async (saveData: CookieGetDetailedInfo): Promise<void> => {
    ...
    if (actualLengthCache > maxElementsFromCache) {
        ...
        Object.keys(cacheTokensCookie).forEach(token => {
            ...
            if (currAddedSeconds < newNearestExpiringToken || !newNearestExpiringToken) {
